-- helper function to set errors in cast function to 0
create or replace function cast_to_float(text, integer) returns float as $$
begin
...
-- Set activeupondischarge type to int
ALTER TABLE eicu_crd.treatment ADD COLUMN activeupondischarge_int varchar;
UPDATE eicu_crd.treatment SET activeupondischarge_int = activeupondischarge;

ALTER TABLE eicu_crd.treatment ALTER activeupondischarge_int TYPE Integer
USING CASE activeupondischarge_int WHEN 'True' THEN 1 ELSE 0 END;


-- Oxygen therapy
 WITH respchart AS (
	SELECT r.patientunitstayid, respchartoffset, respchartvaluelabel, respchartvalue
	FROM eicu_crd.respiratorycharting r

)

, nursechart AS (
	SELECT n.patientunitstayid, nursingchartoffset, nursingchartcelltypevallabel, nursingchartvalue
	FROM eicu_crd.nursecharting n
)

, pat AS (
	SELECT p.patientunitstayid, unitdischargeoffset
...
, ventsettings0 AS (
	...
	FROM (
		...
		SELECT patientunitstayid
			...
			, cast(NULL as integer) AS activeUponDischarge
		FROM nursechart
    	...
    	SELECT t.patientunitstayid
			...
			, activeUponDischarge_int AS activeUponDischarge
	...
	WHERE nursingchartoffset >= -60
		...
		AND cast_to_float(nursingChartValue,0) > 0
		AND cast_to_float(nursingChartValue,0) <= 100
	...
	SELECT patientunitstayid AS icustay_id
		, respchartoffset AS charttime
		, CASE
			WHEN cast_to_float(respchartvalue,0) <= 1 AND cast_to_float(respchartvalue,0) > .22 THEN -1
			WHEN cast_to_float(respchartvalue,0) > 22 THEN -1
			ELSE 0
		END AS oxygen_therapy_type
		, NULL AS activeUponDischarge
	FROM respchart
	WHERE respchartoffset >= -60
		...
		AND (
			cast_to_float(respchartvalue,0) < .2
			OR (
				cast_to_float(respchartvalue,0) > .22
				AND cast_to_float(respchartvalue,0) < 20
			)
			OR cast_to_float(respchartvalue,0) > 22
		)
)
...
(
	SELECT icustay_id
		...
		, CASE
			...
			WHEN (
				cast (MAX(activeUponDischarge) as boolean) 
		...
		, MAX(oxygen_therapy_type) AS oxygen_therapy_type
		, MAX(cast (supp_oxygen as integer)) AS supp_oxygen
	FROM vd2
		LEFT JOIN pat
...
-- keep only first ventilation per ICU stay
, vd4 AS
...
into oxygen_info
from vd4
WHERE vent_start_first = vent_start or vent_start is NULL;
