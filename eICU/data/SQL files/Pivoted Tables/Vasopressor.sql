-- vasopressor usage
-- from: https://github.com/MIT-LCP/eicu-code/blob/master/concepts/pivoted/pivoted-treatment-vasopressor.sql
...
-- take observation closest to ICU admission
drop table if exists vasopressor_info cascade;
create table vasopressor_info as
select distinct p1.patientunitstayid, chartoffset as vasopressor_offset, vasopressor
from pivoted_treatment_vasopressor p1
inner join
	(select patientunitstayid
	 ,min(ABS(chartoffset)) * case when
		 ...
		 THEN -1 ELSE 1 END  as chartoffsetmin -- extract minimum absolut chartoffset
	from pivoted_treatment_vasopressor
	group by patientunitstayid) as p2
on p1.patientunitstayid = p2.patientunitstayid
and p1.chartoffset = p2.chartoffsetmin
